...
# /usr/local/share/org-features/chat/supervisord/ — this script prepares the volume
...
# Guards below no-op cleanly on a container that lacks the stack, so it can also be
# called unconditionally from elsewhere.
...
# The chat stack is installed together or not at all: a container without the chat
# feature (litellm/open-webui/open-terminal/searxng/supervisord) has no supervisord
# binary. Presence of the supervisord binary is therefore the whole signal HERE — this
# script's job is to bring the stack up, and it is also reachable by hand as
# `start_chat_stack`, so it must not second-guess whether the user wants it. WHETHER to
# call it on a container start is decided one level up, by CHAT_AUTOSTART in the
# feature's poststart.sh. Guard the block on both the binary and a LiteLLM config, so a
# chat-less container no-ops.
#
...
if [ -x "$SUPERVISORD_BIN" ] && [ -f "$CONFIG" ]; then
    mkdir -p "$SUP_DATA" \
             "$DATA/litellm" "$DATA/open-terminal/logs" "$DATA/open-terminal/work" \
             "$DATA/searxng" "$DATA/open-webui" "$DATA/open-webui/hf-cache"
...
EOF
    ...
    # per container: SUPERVISORD_DIR (where the conf + wrappers live) and LOGO_NAME
    # (Open WebUI branding). HOME is already exported. Idempotent across restart vs rebuild: if
...
# The chat stack IS installed (supervisord present) but its LiteLLM config is missing,
# so the gate above skipped everything. Say so: this is the one gate failure that means
# something is broken rather than intentionally absent, and it is otherwise invisible —
# the script would exit 0 with no output, exactly like a healthy chat-less container.
# Warn (>&2) instead of exiting non-zero: poststart.sh swallows the status with
# `|| true`, and a non-zero exit under its `set -e` would fail the whole postStart step
# for what is a recoverable, single-service problem.
elif [ -x "$SUPERVISORD_BIN" ]; then
    echo "Warning: chat stack is installed but $CONFIG is missing — not starting it." >&2
    echo "  The installed chat post-create hook generates it from CONFIGURED_PROFILES." >&2
    echo "  Provide profiles/<name>/litellm.json under ORG_DEVCONTAINER_DIR (or" >&2
    echo "  ORG_PROFILES_DIR), then rebuild the container." >&2
fi
# No else for the remaining case: supervisord absent means the chat feature was never
# installed, which is a deliberate, silent no-op.
