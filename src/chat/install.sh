...
# whole stack or none of it (supervisord/start.sh keys its whole block off the
# supervisord binary's presence).
#
...
# hooks in devcontainer-feature.json — a project only lists this feature in its
# devcontainer.json and adds nothing to its own postcreate/poststart.
set -euo pipefail

FEATURE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
. "$FEATURE_DIR/org-build.sh"
org_feature_init "$FEATURE_DIR" "chat"

# Open WebUI pulls opencv-python, whose native module needs libGL at runtime.
if ! dpkg-query -W -f='${Status}' libgl1 2>/dev/null | grep -q 'install ok installed'; then
    apt-get update
...
#
# SSL_CERT_FILE is exported inside the su body so uv/git trust the organizational CAs
...
# resets the env anyway, so it must be set here. The system bundle must already carry
# the custom CAs, installed by the base image before features run.
#
...
# (bump the pin and the feature version, then rebuild).
#
...
su - "${_REMOTE_USER}" -s /bin/bash -c "
    ...
    uv venv --clear --python-preference only-system \"\$VENV\"
