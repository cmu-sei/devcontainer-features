...
# --- Helper: ERE matching a feature's key in devcontainer.json ---
# Either the local "./features/<name>" or the published
# "ghcr.io/cmu-sei/devcontainer-features/<name>[:tag]", so the toggles work whichever
# form the file uses. Feature names are plain [a-z_-], so nothing needs escaping.
feature_ref_re() {
    printf '"(\\./features/%s|ghcr\\.io/cmu-sei/devcontainer-features/%s(:[^"]*)?)"' "$1" "$1"
}

# --- Helper: turn a feature line in devcontainer.json on or off ---
...
# file's existing comments anyway. sed -E (ERE, for the alternation) and [[:space:]]
# behave the same on BSD (macOS) and GNU sed; # is the delimiter since | is taken. Both directions are idempotent: an already-commented
...
set_feature() {
    local name="$1" state="$2" script ref
    [ -f "$DC_JSON" ] || return 0
    ref="$(feature_ref_re "$name")"
    if [ "$state" = "on" ]; then
        script='s#^([[:space:]]*)// *('"$ref"'.*)$#\1\2#'
    else
        script='s#^([[:space:]]*)('"$ref"'.*)$#\1// \2#'
    fi
    if sed -E "$script" "$DC_JSON" > "$DC_JSON.tmp"; then
...
choose_cui() {
    ...
    if [ -f "$TEMPLATE_MARKER" ]; then
        ...
        echo ""
        echo "    curl -fsSL https://github.com/cmu-sei/devcontainer-features/releases/latest/download/spawn.sh | bash -s -- <path>"
        return 0
    fi
...
feature_enabled() {
    grep -qE "^[[:space:]]*$(feature_ref_re "$1")" "$DC_JSON"
}
...
if [ "$SPAWNING" = true ]; then
    ...
    if ! have_tty; then
        ...
        echo "  skipped. Before opening the project, run from its folder:"
        echo ""
        # Relative, not $SCRIPT_DIR: spawn runs this in a staging copy that is moved
        # into place afterwards, so an absolute path would name a directory that is gone.
        echo "    .devcontainer/scripts/setup.sh"
        setup_git
        echo ""
        exit 0
    fi
