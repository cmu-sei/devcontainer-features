...
# Regex matching a feature's key in devcontainer.json: either the local
# "./features/<name>" or the published "ghcr.io/cmu-sei/devcontainer-features/<name>[:tag]",
# so the toggles work whichever form the file uses.
function Get-FeatureRefPattern {
    param([string]$Name)
    $N = [regex]::Escape($Name)
    return "`"(\./features/$N|ghcr\.io/cmu-sei/devcontainer-features/$N(:[^`"]*)?)`""
}
...
function Set-FeatureState {
    ...
    $Ref = Get-FeatureRefPattern $Name
    $Out = @()
    foreach ($Line in (Get-Content $script:DcJson)) {
        if ($On) {
            if ($Line -match "^(\s*)//\s*($Ref.*)$") {
...
function Select-CuiHandling {
    ...
    if (Test-Path $TemplateMarker) {
        ...
        Write-Host ""
        Write-Host "    & ([scriptblock]::Create((irm https://github.com/cmu-sei/devcontainer-features/releases/latest/download/spawn.ps1))) <path>"
        return
    }
...
function Test-FeatureEnabled {
    ...
    $Ref = Get-FeatureRefPattern $Name
    foreach ($Line in (Get-Content $script:DcJson)) {
        if ($Line -match "^\s*$Ref") { return $true }
    }
    return $false
}
...
if ($Spawning) {
    ...
    if (-not (Test-CanPrompt)) {
        ...
        Write-Host "  skipped. Before opening the project, run from its folder:"
        Write-Host ""
        # Relative, not $ScriptDir: spawn runs this in a staging copy that is moved into
        # place afterwards, so an absolute path would name a directory that is gone.
        Write-Host "    .devcontainer\scripts\setup.ps1"
        Initialize-GitRepo
        Write-Host ""
        exit 0
    }
